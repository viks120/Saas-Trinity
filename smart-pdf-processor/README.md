...
A subscription-based web application that enables users to upload PDF documents and extract text content with tier-based processing limits. Built with FastAPI, React, PostgreSQL, and Docker.

## Overview

The Smart PDF Processor provides:
- 📄 PDF upload and text extraction with paragraph structure preservation
- 💳 Tier-based word limits (Free: 100 words, Pro: 200 words, Enterprise: unlimited)
- 📚 Personal document library with status tracking
- 🖼️ Image marker insertion for visual content locations
- ⚡ Background processing with status updates
- 📥 Download extracted text as plain text files
- 🔐 Secure cookie-based authentication with session binding
- 👨‍💼 Admin panel for managing users and tiers
- 🐳 Docker-based deployment with hot-reload for development

## Features

### PDF Processing
- **Upload PDFs**: Upload PDF files up to 10MB in size
- **Text Extraction**: Automatically extract text while preserving paragraph structure
- **Image Detection**: Insert `**[IMAGE]**` markers where images appear in the PDF
- **Tier-Based Limits**: 
  - Free tier: First 100 words
  - Pro tier: First 200 words
  - Enterprise tier: Complete document (unlimited)
...
### Document Management
- **Document Library**: View all your uploaded documents
- **Document Details**: View extracted text with preserved formatting
- **Download**: Download extracted text as `.txt` files
- **Delete**: Remove documents and their associated files
...
Returns plain text file with extracted content.
...
- `409 Conflict`: Document not ready for download
...
- Check backend logs: `docker compose logs pdf-backend`
- Verify background task is running
- Restart backend: `docker compose restart pdf-backend`

**Processing fails with error:**
- Check if PDF is corrupted or password-protected
- Verify PDF is not scanned images (OCR not supported)
- Check backend logs for detailed error messages

**File upload rejected:**
- Ensure file is a valid PDF (not renamed image)
- Check file size is under 10MB
- Verify user is authenticated

**Word limit not applied correctly:**
- Check user's tier configuration in database
- Verify tier features include `pdf_word_limit`
- Run seed script to update tier configurations
